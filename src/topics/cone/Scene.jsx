...
export default function ConeScene({ values, autoPlay, currentStep }) {
  ...
  const heightLine = [new THREE.Vector3(0, -h/2, 0), new THREE.Vector3(0, h/2, 0)]
  const radiusLine = [new THREE.Vector3(0, -h/2, 0), new THREE.Vector3(r, -h/2, 0)]
  const slantLine  = [new THREE.Vector3(0, h/2, 0), new THREE.Vector3(r, -h/2, 0)]
  ...
  return (
    <group>
      <group rotation={[0, rotY.current, 0]}>
        {/* Solid */}
        {showSolid && (
          <mesh position={[0, 0, 0]} scale={[r * ep, h * hEp, r * ep]}>
            <coneGeometry args={[1, 1, 64]} />
            <meshStandardMaterial color="#f59e0b" transparent opacity={0.28}
              emissive="#f59e0b" emissiveIntensity={0.1} roughness={0.2} metalness={0.3} />
          </mesh>
        )}
        {/* Wire */}
        <mesh position={[0, 0, 0]} scale={[(r+0.01)*ep, (h+0.01)*hEp, (r+0.01)*ep]}>
          <coneGeometry args={[1, 1, 32]} />
          <meshBasicMaterial color="#fbbf24" wireframe transparent opacity={0.26} />
        </mesh>
        {/* Base ring */}
      ...
      {/* Axis / Height Line (Internal) */}
      <Line points={heightLine} color="#ec4899" lineWidth={2} transparent opacity={ep} />
      
      {/* Radius Line */}
      <Line points={radiusLine} color="#10b981" lineWidth={2} transparent opacity={ep} />

      {/* Slant Height Line */}
      <Line points={slantLine} color="#a78bfa" lineWidth={2} transparent opacity={ep} />

      {showLabels && ep > 0.5 && (
        <>
          <Html className="r3f-html" position={[0, 0, 0]} center style={{ pointerEvents:'none' }} >
            <div style={TAG('#be185d')}>h = {h.toFixed(1)}</div>
          </Html>
          <Html className="r3f-html" position={[r/2, -h/2 - 0.4, 0]} center style={{ pointerEvents:'none' }} >
            <div style={TAG('#047857')}>r = {r.toFixed(1)}</div>
          </Html>
          <Html className="r3f-html" position={[r/2 + 0.5, 0, 0]} center style={{ pointerEvents:'none' }} >
            <div style={TAG('#6d28d9')}>l = {l.toFixed(1)}</div>
          </Html>
        </>
      )}
      ...
      {showLabels && ep > 0.7 && (
        <Html className="r3f-html" position={[-r-1.7, 0, 0]} center style={{ pointerEvents:'none' }} >
          <div style={{ background:'rgba(255,255,255,0.95)', border:'1px solid rgba(245,158,11,0.3)',
            borderRadius:'12px', padding:'10px 16px', minWidth:'160px', boxShadow:'0 4px 16px rgba(0,0,0,0.09)' }}>
            <div style={{ color:'#94a3b8', fontSize:'10px', marginBottom:'6px', fontFamily:'Fira Code,monospace', letterSpacing:'0.05em' }}>CONE</div>
            <div style={{ color:'#f59e0b', fontSize:'13px', fontFamily:'Fira Code,monospace', lineHeight:1.9, fontWeight:700 }}>
